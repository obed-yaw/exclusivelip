...
class product_class extends db_connection
{
    ...
    public function display_one_product($id){
        $sql = "SELECT `products`.`id`, `products`.`product_cat`, `categories`.`cat_name`, `products`.`product_name`, `products`.`stock`, `products`.`product_price`, `products`.`product_desc`, `products`.`product_img`, `products`.`product_img`, `products`.`seller_id`, `users`.`username` FROM `products` JOIN `users` ON (`products`.`seller_id` = `users`.`id`) JOIN `categories` ON (`products`.`product_cat` = `categories`.`id`) WHERE `products`.`id` = '$id'";
        return $this->db_query($sql);
    }
